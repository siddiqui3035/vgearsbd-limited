<x-front-app-layout>
                                        ...
                                        @forelse ($carts as $cart)
                                            <tr>
                                                ...
                                                <td class="product_thumb"><a href="{{ route('products') }}"><img
                                                            src="{{ asset($cart->product->thumble) }}"
                                                            alt=""></a>
                                                </td>
                                                <td class="product_name"><a
                                                    href="#">{{ $cart->product->name }}</a></td>
                                                <td class="product-price">
                                                    BDT&nbsp;<span class="price">{{ $cart->product->purchase_price }}</span>.00</td>
                                                <form action="{{ route("cart.update", $cart->id) }}" method="post" class="update-cart">
                                                    <td class="product_quantity"><label>Quantity</label>
                                                        @csrf
                                                        @method("PUT")
                                                        <input name="product_qty"
                                                        min="1" max="100" value="{{ $cart->product_qty }}"
                                                        type="number" class="quantity">
                                                    </td>
                                                </form>
                                                <td class="product_total">
                                                    BDT&nbsp;<span class="total_price">{{ $cart->product->purchase_price * $cart->product_qty }}</span>.00
                                                    <div class="d-none">
                                                    {{ $price += $cart->product->purchase_price * $cart->product_qty }}
                                                    </div>
                                                </td>
                                            </tr>
                                        @empty
                            ...
                            <div class="coupon_code right">
                                ...
                                <div class="coupon_inner">
                                    ...
                                    <div class="cart_subtotal">
                                        <p>Total</p>
                                        <p class="cart_amount">
                                            BDT&nbsp;<span class="net_price">{{ $price +120 }}</span>.00 {{-- TODO: Add shiipping cost dinamically --}}
                                        </p>
                                    </div>
    ...
    <script>
        ...
        $('.quantity').on("keyup change", function(){
            $(this).closest('tr').find('.total_price').html(parseInt($(this).val()) * parseInt($(this).closest('tr').find('.price').html()))
            let gross_price = 0;
